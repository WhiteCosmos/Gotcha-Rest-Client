...
class GHCJsonNodeBuilder {
        ...
        if (typeof json === 'object' && !Array.isArray(json)) {
            ...
            for (let key in json) {
                ...
                if (BigNumber.isBigNumber(value)) {
                    node = JsonNode.newPrimitiveNode(key, 'number', value.toString())
                    node.layer = jsonNode.layer + 1
                }
    ...
    buildJsonFromJsonNode(jsonNode) {
        ...
        if (jsonNode.type === 'string') {
            return this.dynamicVariableRender.render(jsonNode.value)
        }
        ...
        if (jsonNode.type === 'number') { // 可能存在溢出和转换问题
            try {
                ...
                if (this.dynamicVariableRender) {
                    value = this.dynamicVariableRender.render(jsonNode.value)
                }

                return BigNumber(value) //统一转换为BigNumber
            } catch (err) {
                console.error("jsonNode.type === 'number'", err)
