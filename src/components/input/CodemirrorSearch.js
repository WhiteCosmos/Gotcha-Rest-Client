...
(function (mod) {
    ...
    function searchOverlay(cm, query, caseInsensitive) {
    ...
    function startSearch(cm, state, query) {
        ...
        state.overlay = searchOverlay(cm, state.query, queryCaseInsensitive(state.query))
        cm.addOverlay(state.overlay)
        CodeMirror.signal(cm, 'searchComplete', state)
    ...
    function doSearch_(cm, rev) {
        let state = getSearchState(cm)

        state.query = state.lastQuery

        if (state.query) {
            startSearch(cm, state, state.query)
            // findNext(cm, rev)
        }
    ...
    CodeMirror.commands.find_ = function (cm) {
        clearSearch(cm)
        doSearch_(cm)
    }
    ...
    CodeMirror.commands.findNext_ = function (cm) {
        findNext(cm, false)
    }

    CodeMirror.commands.findPrev_ = function (cm) {
        findNext(cm, true)
    }
