        ...
        <v-card>
            <div class="ev-remove-project-modal">
                <div class="ev-remove-project-modal__message-container">
                    <span class="ev-remove-project-modal__text">{{ $t('EvRemoveProjectModal.message', {name: name}) }}</span>
                </div>

                <div class="ev-remove-project-modal__button-container">
                    <v-spacer></v-spacer>

                    <v-btn color="error" small @click="confirm" elevation="0">{{ $t('EvRemoveProjectModal.confirm') }}</v-btn>

                    <ev-vertical-gutter :gutter="8"></ev-vertical-gutter>

                    <v-btn color="grey" small text @click="cancel">{{ $t('EvRemoveProjectModal.cancel') }}</v-btn>
                </div>
            </div>
        </v-card>
    </v-dialog>
...
<script>
    ...
    export default {
        ...
        created() {
            AppEventBus.$on(AppEvents.REMOVE_PROJECT, () => {
                this.dialog = true
            })
        },
